...
<?php
    ...
    $staff = "select * from staff where staff_id='$staff_id'";
    $run_staff = mysqli_query($con, $staff);
    $row_staff = mysqli_fetch_array($run_staff);
    $staff_job = $row_staff['staff_job'];

    $select = "select * from attendance where user_id='$staff_id' and user_type='staff'";
...
?>
    <body>
       <div class="row">
           <div class="col-sm-12">
               <center><h2>Monthly Payment</h2></center>
           </div>
       </div><br>
       <div class="row">
           <div class="col-sm-5">
            ...
            <div class="col-sm-5">
                <?php
                    ...
                    $day = (int)$day;
                    $month = (int)$month;
                    $month_ab = $values[$month-1];
                    $working_days = $day - $month_ab;
                    if($staff_job == 'mess'){
                        $per_day = $daily_mess_worker_price;
                        $amount = $daily_mess_worker_price*(30 - $month_ab);
                    }
                    else{
                        $per_day = $daily_room_service_price;
                        $amount = $daily_room_service_price*(30 - $month_ab);
                    }
                    echo"
                        <center style='font-size:25px'><strong>Total absent days in this month: &nbsp;</strong>$month_ab</center><br>
                        <center style='font-size:25px'><strong>Total Working days in this month: &nbsp;</strong>$working_days</center><br>
                        <center style='font-size:25px'><strong>Charges per each day : &nbsp;</strong>$per_day</center><br>

                        <div class='pay'>
                            <center class='center'>Expected Amount at month end: $amount</center>
                        </div>
                    ";
                ?>
